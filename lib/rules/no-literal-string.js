...
module.exports = {
  ...
  create: function (context) {
    ...
    const scriptVisitor = {
      ...
      'TSModuleDeclaration Literal'(node) {
      ...
      'Literal:exit'(node) {
        ...
        context.report(
          ...
          {
            node,
            message: "Forbidden literal string: {{ code }}.  Parent types: {{ ptype }}",
            data: {
              code: context.getSourceCode().getText(node),
              ptype: JSON.stringify(context.getAncestors(node).map(n => n.type)),
            },
          });
      }
    };
